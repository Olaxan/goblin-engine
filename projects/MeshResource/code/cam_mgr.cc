...
namespace efiilj
{
	...
	: ubo_(0), active_camera_(0), aspect_(float(width) / float(height)), up_(vector3(0, 1, 0))
	{
		auto trf_ptr = std::make_shared<transform_model>(vector3(0, 0, 0), vector3(0), vector3(1, 1, 1));
		auto cam = std::make_shared<camera_model>(DEFAULT_FOV, aspect_, DEFAULT_NEAR, DEFAULT_FAR, trf_ptr, up_);
